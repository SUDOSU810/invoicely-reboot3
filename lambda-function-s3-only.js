...
export const handler = async (event) => {
  ...
  // --- ALSO Upload Invoice Metadata as JSON to S3 ---
  const metadataKey = `invoices/metadata/${userId}_${invoiceId}.json`;
  const metadata = {
    userId,
    invoiceId,
    ...
    pdfKey: pdfKey, // Store S3 key instead of signed URL
  ...
  return {
    ...
    headers: corsHeaders,
    body: JSON.stringify({
      message: "✅ Invoice created and uploaded to S3 successfully!",
      pdfUrl,
      invoice: {
        ...metadata,
        pdfUrl // Include signed URL in response
      }
    }),
  };
};