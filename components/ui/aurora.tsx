...
const AuroraComponent = (props: AuroraProps) => {
  const {
    ...
    colorStops: initialColorStops = ["#1e3a8a", "#3b82f6", "#60a5fa"], // Blue shades
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (!gl) {
        ...
        ctn.style.background = `linear-gradient(45deg, ${initialColorStops[0]} 0%, ${initialColorStops[1]} 50%, ${initialColorStops[2]} 100%)`;
      ...
      console.error('Aurora component error:', error);
      // Fallback to CSS gradient
      ctn.style.background = `linear-gradient(45deg, ${initialColorStops[0]} 0%, ${initialColorStops[1]} 50%, ${initialColorStops[2]} 100%)`;
      ctn.style.opacity = '0.3';
    }
