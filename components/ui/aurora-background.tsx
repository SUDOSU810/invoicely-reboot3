...
const AuroraBackground = () => {
  ...
  useEffect(() => {
    ...
    const fragmentShaderSource = `
      ...
      void main() {
        ...
        for (int i=0; i<MARCH_STEPS; i++) {
            ...
            float density = map(p);
            if (density > 0.0) {
                // Blue aurora colors - deep blue to light blue
                vec3 auroraColor = vec3(0.1, 0.3, 0.8) + 0.3 * cos(iTime * 0.5 + p.y * 2.0 + vec3(0.0, 0.5, 1.0));
    ...
    const renderLoop = () => {
      ...
      gl.uniform1f(uFlowSpeedLocation, 0.4);
      gl.uniform1f(uColorIntensityLocation, 1.2);
      gl.uniform1f(uNoiseLayersLocation, 4.0);
      gl.uniform1f(uMouseInfluenceLocation, 0.3);
  ...
  return (
    <div className="fixed inset-0 w-full h-full z-0" aria-hidden>
      <canvas ref={canvasRef} className="absolute top-0 left-0 w-full h-full" />
      <div className="pointer-events-none absolute inset-0 bg-gradient-to-t from-blue-900/20 via-transparent to-blue-800/10" />
    </div>
  );
};
